...
public class OldEllipse {
	...
	protected OldEllipse() {}
	
	public OldEllipse(Graph g,Color boundary,int x_centre,int y_centre,int a,int b) {
		this.g = g;
	...
	public OldEllipse(Graph g,Color boundary,Color fill,int x_centre,int y_centre,int a,int b) {
		this.g = g;
		this.boundary = boundary;
		this.fill = fill;
		this.x_centre=x_centre;
	...
	public void getFillEllipse() {
		...
		if(centreColor!=null) {
			colors[x_centre+g.getX_size()][y_centre+g.getY_size()]=3;
		}
	}
	...
	private void fillinside(int x,int y) {
		...
		if(x==0 || y==0) {
			colors[x+g.getX_size()][y+g.getY_size()]=-1;
		}
	...
	private void fillOnGraph() {
		...
		for(int i=0;i<colors.length;i++) {
			for(int j=0;j<colors[0].length;j++) {
				if(colors[i][j]==2) {
					g.plotPoint(i-n,j-m,this.fill,this.fillThickness==-1?10:fillThickness);
				}
			}
		}
	}
	...
	private void drawOnGraph() {
		...
		for(int i=0;i<colors.length;i++) {
			for(int j=0;j<colors[0].length;j++) {
				if(colors[i][j]==1) {
					g.plotPoint(i-n,j-m,this.boundary,this.borderThickness==-1?10:borderThickness);
				}
			}
		}
	}
		...
		for(int i=0;i<colors.length;i++) {
			for(int j=0;j<colors[0].length;j++) {
				if(colors[i][j]==1||colors[i][j]==2||colors[i][j]==3) {
					double mod_x = i - g.getX_size() - x_pivot;
					double mod_y = j -g.getY_size()- y_pivot;
					...
					new_x+=g.getX_size();
					new_y+=g.getY_size();
					if(new_x<2*g.getX_size()&&new_y<2*g.getY_size()) {
	...
	public void drawRotatedScaledEllipse() {
		drawOnGraph();
	}
	
	public void fillRotatedScaledEllipse() {
		drawOnGraph();
		fillOnGraph();
	}
	...
	public void scaleEllipse(int scale_x,int scale_y) {
		...
		for(int i=0;i<colors.length;i++) {
			for(int j=0;j<colors[0].length;j++) {
				if(colors[i][j]==1||colors[i][j]==2||colors[i][j]==3) {
					int xcoord = i-g.getX_size();
					int ycoord = j-g.getY_size();
					int newx = xcoord * scale_x;
					int newy = ycoord*scale_y;
					newx+=g.getX_size();
					newy+=g.getY_size();
					if(newx<2*g.getX_size()&&newy<2*g.getY_size()) {
